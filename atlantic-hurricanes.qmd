...
So far we have `max wind speed` and `time` as potential factors driving the cost of storm `damage costs`. It is likely that there are a few more. Perhaps rainfall has an effect?
...
```
Our p-value is greater than $alpha = 0.05$, so we cannot reject the null hypothesis; we cannot rule out that the effect of `time` on storm `damage costs` is due to random chance.
...
places <- ggplot(hurricane_data_cleaned, aes(x = total_areas, y = damage_mil)) +
  ...
  labs(x = "Number of Areas Affected",
       y = "Damage (Millions of USD)",
       title = "Cost by Areas Affected") +
  theme_minimal()

# Grid plots
wind + rain + time + places
```

## Log Damage 

Since `damage costs` span a wide range (up to $125,000,000,000), and violate the normality assumption, we will run the analysis on the *log* of `damage costs` to reduce the distance between some of these extreme points.
...
Our damage cost data is right-skewed. Adding a log transformation makes these data more symmetric. 
...
log_places <- ggplot(hurricane_data_cleaned, aes(x = total_areas, y = log(damage_mil))) +
  ...
  labs(x = "Number of Areas Affected",
       y = "Log Damage (Millions of USD)",
       title = "Log Cost by Areas Affected") +
  theme_minimal()

log_wind + log_rain + log_time + log_places
```
