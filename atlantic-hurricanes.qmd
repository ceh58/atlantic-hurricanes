...
Hurricanes are assigned categories 1 through 5 on the Saffir-Simpson scale by wind speed[^2]. Storms given a rating of 3 or higher are considered "major hurricanes" and are expected to bring catastrophic damage (and thus more significant damage costs). While the frequency of storms is not projected to change, they are predicted to become more intense[^3].

**As storms increase in intensity over time, will the damage costs also be driven by time?**
...
[^3]: Read more about how hurricanes are expected to evolve with climate change [here](https://science.nasa.gov/earth/climate-change/a-force-of-nature-hurricanes-in-a-changing-climate/).

# Data

I will be using Kaggle's [North American Hurricanes from 2000](https://www.kaggle.com/datasets/middlehigh/north-american-hurricanes-from-2000) dataset to develop a model of the factors driving storm `damage costs`, including `max wind speed`, `rainfall`, and `time`. Of these factors:
...
save_kable(na_table, here("graphs", "na_table.jpg"))
```
It appears that removing the NA values will not have an impact on the analysis.

Next, I will scale the `damage costs` values to be represented by "Millions of USD."
```{r}
#| code-summary: Expand Code
...
save_kable(mean_median, here("graphs", "mean_median_table.jpg"))
```
The `damage costs` data appear to be right-skewed across all hurricane categories. Mean damage costs are higher than the median damage costs due to more weight/outliers on the right tail (high end) of the observations. Thus, we may want to log-transform these data.
...
ggsave(here("graphs", "damage_over_years.jpg"))
```
...
ggsave(here("graphs", "damage_years_category.jpg"))
```
...
ggsave(here("graphs", "wind_years.jpg"))
```
You can see 8 category 5 storms (with 157+ winds) between 2015 and 2023, as compared to just 2 from 2000 to 2014.

So far `max wind speed` and `time` are interesting potential factors driving storm `damage costs`. It is likely that there are a few more. Perhaps rainfall, which is also expected to increase with climate change, has an effect?
```{r}
#| code-summary: Expand Code
# Rainfall over time
...
ggsave(here("graphs", "rain_years.jpg"))
```
...
## Plots
Individual relationships with `damage costs`.
```{r}
#| code-summary: Expand Code
...
ggsave(here("graphs", "normal_grid.jpg"))

```

## Multiple Regression Model (No Transformation)
...
ggsave(here("graphs", "normal_beta1_density.jpg"))
```
...
ggsave(here("graphs", "normal_beta2_density.jpg"))
```
...
ggsave(here("graphs", "normal_beta3_density.jpg"))
```
...
ggsave(here("graphs", "hist_damage.jpg"))
```
...
ggsave(here("graphs", "log_damage.jpg"))
```

To see if the log damage model is a better fit, we can use a Scale-Location[^4] plot to observe the residuals.  

[^4]: Adapted from [here](https://library.virginia.edu/data/articles/interpreting-log-transformations-in-a-linear-model).

## Multiple Regression Model (With Transformation)
```{r}
#| code-summary: Expand Code
# Create the model
...
ggplot(hurricane_data_cleaned, aes(sample = damage_mil)) +
  geom_qq() +
  geom_qq_line() +
  theme_minimal()

ggplot(hurricane_data_cleaned, aes(sample = log(damage_mil))) +
  geom_qq() +
  geom_qq_line() +
  theme_minimal()
```
...
## Plots
Individual relationships with log(`damage costs`).
```{r}
#| code-summary: Expand Code
...
ggsave(here("graphs", "log_grid.jpg"))
```
...
ggsave(here("graphs", "log_beta1_density.jpg"))
```
...
ggsave(here("graphs", "log_beta2_density.jpg"))
```
...
ggsave(here("graphs", "log_beta3_density.jpg"))
```
